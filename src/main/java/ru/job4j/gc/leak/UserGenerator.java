...
public class UserGenerator implements Generate {
    ...
    public void generate() {
        ...
        for (int i = 0; i < NEW_USER; i++) {
            ...
            stringJoiner.add(surnames.get(random.nextInt(surnames.size())));
            stringJoiner.add(names.get(random.nextInt(names.size())));
            stringJoiner.add(patrons.get(random.nextInt(patrons.size())));
            users.add(new User(stringJoiner.toString()));
        }
    }
